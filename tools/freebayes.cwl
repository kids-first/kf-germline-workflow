...
inputs:
  ...
  output_gvcf: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--gvcf" : "")' }, doc: "Write gVCF output, which indicates coverage in uncalled regions." }
  ...
  only_use_input_alleles: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--only-use-input-alleles" : "")' }, doc: "Only provide variant calls and genotype likelihoods for sites and alleles which are provided in the VCF input, and provide output in the VCF for all input alleles, not just those which have support in the data." }
  ...
  report_all_haplotype_alleles: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--report-all-haplotype-alleles" : "")' }, doc: "At sites where genotypes are made over haplotype alleles, provide information about all alleles in output, not only those which are called." }
  report_monomorphic: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--report-monomorphic" : "")' }, doc: "Report even loci which appear to be monomorphic, and report all considered alleles, even those which are not in called genotypes. Loci which do not have any potential alternates have '.' for ALT." }
  ...
  strict_vcf: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--strict-vcf" : "")' }, doc: "Generate strict VCF format (FORMAT/GQ will be an int)" }
  ...
  pooled_discrete: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--pooled-discrete" : "")' }, doc: "Assume that samples result from pooled sequencing. Model pooled samples using discrete genotypes across pools. When using this flag, set --ploidy to the number of alleles in each sample or use the --cnv-map to define per-sample ploidy." }
  pooled_continuous: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--pooled-continuous" : "")' }, doc: "Output all alleles which pass input filters, regardles of genotyping outcome or model." }

  # Reference Allele Variables

  use_reference_allele: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--use-reference-allele" : "")' }, doc: "This flag includes the reference allele in the analysis as if it is another sample from the same population." }
  ...
  no_partial_observations: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--no-partial-observations" : "")' }, doc: "Exclude observations which do not fully span the dynamically-determined detection window.  (default, use all observations, dividing partial support across matching haplotypes when generating haplotypes.)" }

  # Indel Realignment Variables

  dont_left_align_indels: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--dont-left-align-indels" : "")' }, doc: "Turn off left-alignment of indels, which is enabled by default." }

  # Input Filter Variables

  use_duplicate_reads: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--use-duplicate-reads" : "")' }, doc: "Include duplicate-marked alignments in the analysis. default: exclude duplicates marked as such in alignments" }
  ...
  standard_filters: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--standard-filters" : "")' }, doc: "Use stringent input base and mapping quality filters. Equivalent to --min-mapping-quality 30 --min-base-quality 20 --min-supporting-allele-qsum 0 --genotype-variant-threshold 0" }
  ...
  no_population_priors: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--no-population-priors" : "")' }, doc: "Equivalent to --pooled-discrete --hwe-priors-off and removal of Ewens Sampling Formula component of priors." }

  # Mappability Priors

  hwe_priors_off: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--hwe-priors-off" : "")' }, doc: "Disable estimation of the probability of the combination arising under HWE given the allele frequency as estimated by observation frequency." }
  binomial_obs_priors_off: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--binomial-obs-priors-off" : "")' }, doc: "Disable incorporation of prior expectations about observations. Uses read placement probability, strand balance probability, and read position (5'-3') probability." }
  allele_balance_priors_off: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--allele-balance-priors-off" : "")' }, doc: "Disable use of aggregate probability of observation balance between alleles as a component of the priors." }
  ...
  legacy_gls: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--legacy-gls" : "")' }, doc: "Use legacy (polybayes equivalent) genotype likelihood calculations" }
  ...
  report_genotype_likelihood_max: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--report-genotype-likelihood-max" : "")' }, doc: "Report genotypes using the maximum-likelihood estimate provided from genotype likelihoods." }
  ...
  exclude_unobserved_genotypes: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--exclude-unobserved-genotypes" : "")' }, doc: "Skip sample genotypings for which the sample has no supporting reads." }
  ...
  use_mapping_quality: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--use-mapping-quality" : "")' }, doc: "Use mapping quality of alleles when calculating data likelihoods." }
  harmonic_indel_quality: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--harmonic-indel-quality" : "")' }, doc: "Use a weighted sum of base qualities around an indel, scaled by the distance from the indel.  By default use a minimum BQ in flanking sequence." }
  ...
  genotype_qualities: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--genotype-qualities" : "")' }, doc: "Calculate the marginal probability of genotypes and report as GQ in each sample field in the VCF output." }

  # Debugging Variables

  debug: { type: 'boolean?', default: false, inputBinding: { valueFrom: '$(self ? "--debug" : "")' }, doc: "Print debugging output." }
