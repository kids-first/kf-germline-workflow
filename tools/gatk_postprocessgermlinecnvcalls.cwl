  ...
  - position: 0
    ...
    valueFrom: >-
      ...
      /gatk --java-options "-Xmx${return Math.floor(inputs.max_memory*1000/1.074-1)}m" PostprocessGermlineCNVCalls \\
          $calls_args \\
          $model_args \\
          ${var arr=[]; for (var x = 0; x < inputs.allosomal_contigs_args.length; x++) {arr.push(inputs.allosomal_contigs_args[x])}; return arr.join(' ')} \\
...
outputs:
  genotyped_intervals_vcf: { type: File, outputBinding: { glob: 'genotyped-intervals-*' } }
  genotyped_segments_vcf: { type: File, outputBinding: { glob: 'genotyped-segments-*' } }
