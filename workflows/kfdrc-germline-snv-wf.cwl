...
inputs:
  ...
  break_bands_at_multiples_of: {type: 'int?', default: 1000000, doc: "If set to a positive value will create a new interval list with
      the original intervals broken up at integer multiples of this value. Set to 0 to NOT break up intervals."}
...
outputs:
  ...
  gatk_hardfiltered_vcf: {type: 'File?', doc: "VCF containing the all genotyping variants with their hardfilters", outputSource: single_sample_genotyping/hardfiltered_vcf}
...
steps:
  ...
  single_sample_genotyping:
    ...
    in:
      ...
      cadd_snvs: cadd_snvs
      intervar: intervar
    out: [collectvariantcallingmetrics, peddy_html, peddy_csv, peddy_ped, vep_annotated_vcf, annotation_plots, hardfiltered_vcf]
hints:
