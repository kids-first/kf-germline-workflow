...
doc: |
  # Kids First DRC GATK gCNV Case Workflow
...
inputs:
  ...
  gcnv_model_tars: {type: 'File[]', doc: "Array of tars of the contig-ploidy model\
      ...
      \ workflow.", "sbg:fileTypes": "TAR.GZ"}
  output_basenames: {type: 'string[]?', doc: "Override output naming for each of the normal_reads provided. Must provide one string per normal_reads file."}
  ...
  scatter_ploidy_calls_ram: {type: 'int?', doc: "GB of RAM to allocate to scatter\
      \ ploidy calls"}
  scatter_ploidy_calls_cores: {type: 'int?', doc: "Minimum reserved number of CPU\
      \ cores for scatter ploidy calls"}
...
steps:
  ...
  germline_cnv_caller_case:
    ...
    scatter: [scatter_index, gcnv_model_tar]
    scatterMethod: dotproduct
    in:
      scatter_index: index_tars_array/index_array
      gcnv_model_tar: gcnv_model_tars
    ...
    out: [gcnv_call_tars, gcnv_tracking_tar, calling_config_json, denoising_config_json,
      gcnvkernel_version_json, sharded_interval_list]
  ...
  index_normal_reads_array:
    run: ../tools/expression_create_index_array.cwl
    in:
      array: normal_reads
    out: [index_array]

  clt_string_array_pickval:
    run: ../tools/clt_string_array.cwl
    in:
      input:
        source: [output_basenames, collect_read_counts/entity_id]
  ...
  postprocess_gcnv_and_collectsamplequalitymetrics:
    ...
    scatterMethod: dotproduct
    in:
      entity_id: clt_string_array_pickval/output
      ...
      sample_index: index_normal_reads_array/index_array
